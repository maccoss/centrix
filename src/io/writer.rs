...
///
/// Empty centroid results (zero peaks) ARE still written to the output with
/// `defaultArrayLength="0"`. This is intentional: the `<chromatogramList>`
/// TIC has a `defaultArrayLength` equal to the total scan count, and DIA-NN
/// cross-references the TIC against `<spectrumList>` entries. Dropping
/// spectra would create a mismatch that crashes DIA-NN with an access
/// violation.
